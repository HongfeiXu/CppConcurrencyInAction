...
- **两阶段笔记**：先按自己的节奏**读完并练完一整章**，在 `docs/study_notes/` 用草稿记下要点与疑问；**不要**边读边追求笔记完美，以免打断心流。
- **章末再打磨**：一章结束后，把该章笔记交给 AI 助手优化（例如用 Cursor `@` 对应 `chapterNN_*.md`），可说明偏好：更简洁、更贴原书结构、术语与缩写**首次出现带英文**、表格化对比等。
- **优化时可选说明**：本章最想巩固什么、哪些段落是摘抄、哪些是存疑，便于对症修订；个人代码示例与实验结论一般保留，除非有明显错误。
